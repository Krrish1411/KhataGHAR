...
import { Lock, Unlock, Shield, Plus, Upload, KeyRound, CheckCircle2, Sparkles, ArrowLeftRight } from 'lucide-react';
...
export const LockScreen: React.FC = () => {
  ...
  return (
    <div
      style={{ zoom: 1.15 }}
      className="min-h-screen min-h-[100dvh] bg-ground text-ink flex flex-col items-center justify-center p-4 sm:p-6 relative select-none selection:bg-pine-500/20 selection:text-pine-600"
    >
      <div className="w-full max-w-lg space-y-6">
        {/* Brand Header */}
        <div className="text-center space-y-2">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-pine-600 text-white shadow-button-primary mb-1">
            <Lock className="w-8 h-8 stroke-[2.2]" />
          </div>
          <h1 className="text-3xl font-extrabold tracking-tight text-ink font-display" translate="no">
            KhataGHAR
          </h1>
          <p className="text-xs sm:text-sm text-ink/65 font-medium">
        ...
        {/* Unlock Card */}
        <div
          className={`bg-card border border-line rounded-3xl p-7 sm:p-8 shadow-card transition-transform lift ${
            shake ? 'animate-shake' : ''
          }`}
        >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2.5 truncate">
                        <Shield className={`w-4 h-4 flex-shrink-0 ${isSelected ? 'text-pine-600 dark:text-pine-400' : 'text-ink/40'}`} />
                        <span className="text-xs sm:text-sm font-semibold truncate">{vault.name}</span>
                        {vault.isPrimary && (
                          <span className="text-[10px] bg-card text-ink/60 px-1.5 py-0.5 rounded font-normal border border-line">
                            Primary
                          </span>
                        )}
                      </div>
                      {isSelected && <CheckCircle2 className="w-4 h-4 text-pine-600 dark:text-pine-400 flex-shrink-0" />}
                    </button>
                  );
                })}
              </div>
          ...
          <form onSubmit={handleUnlock} className="space-y-4">
            ...
            <Button
              ...
              variant="primary"
              size="lg"
              className="w-full py-3 sm:py-3.5 text-sm sm:text-base font-bold shadow-md shadow-pine-900/20"
          ...
          {/* Secondary Options */}
          <div className="mt-5 pt-3.5 border-t border-line flex items-center justify-between text-xs sm:text-[13px] text-ink/60">
            <button
              onClick={() => setIsCreateOpen(true)}
              className="flex items-center gap-1.5 hover:text-pine-600 font-semibold transition-colors cursor-pointer"
            >
          ...
          <div className="mt-3.5 pt-3 border-t border-line text-center">
            <button
              ...
              className="text-xs sm:text-[13px] font-bold text-pine-600 dark:text-pine-400 hover:underline inline-flex items-center gap-1.5 cursor-pointer"
            >
